...
cb_clad <- cb_wide %>% dplyr::select(survey_code:cb_volume, mm, allcladocera, allcladocera_log, -core)
...
#cb_clad_sta <- dplyr::inner_join(cb_clad, stations_sf, by=c("station")) # all stations

# make into sf spatial object:
#cb_clad_sf <- st_as_sf(cb_clad_sta, coords = c("lon","lat"), crs=4326, remove=FALSE) # all stations
cb_clad_crop <- st_as_sf(cb_clad_crop, coords = c("lon","lat"), crs=4326, remove=FALSE)

# mapofsites <- mapview(cb_clad_crop, col.regions="yellow", layer.name="Sites_Cropped") + mapview(delta_crop, col.regions="blue", alpha.regions=0.4, legend=F) 

# all stations
# (mapstations <-  mapview(cb_clad_crop, col.regions="yellow", layer.name="Sites_Cropped") + mapview(cb_clad_sf, col.regions="green", alpha=0.2, cex=0.3, layer.name="Sites_full"))
...
cb_clad_crop <- cb_clad_crop %>% #cut(das$anim, 3, labels=FALSE)
  mutate(allclad_cut = as.integer(Hmisc::cut2(allcladocera, g=5)))
